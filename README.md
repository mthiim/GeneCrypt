...
The cryptographic aspects have been deliberately kept simple to illustrate the concept more clearly. More refined protocols would be used in practice.

What makes this an interesting application of confidential computing principles, is first of all the dilemma of both protecting genomic data while also allowing it to be used to the benefit of the individual. Unlike many other contexts, in this use case, you have a massive amount of data that is sensitive, but where you don’t need all the raw data in a practical situation, but merely some computationally derived value. This could, for instance, be a boolean value indicating the presence or absence of some genetic variant. Enclaves lend themselves well to be the filters that extract just the relevant information.
Confidential computing allows users to grant much more granular control than "I give doctors X, Y, Z access to my whole genome". With GeneCrypt, the user can be presented with exactly what information is revealed and to whom. Only the enclaves approved by the user gets access to the data, and this in a way that is fully transparent and verifiable to the user. Users are no longer at the mercy of the security practices of the medical facility and their IT systems but have real, actual control of their health data. Another compelling feature of this use case is that it doesn’t require users themselves to have special hardware (with e.g. SGX support) for this to work. The hardware support needs to be at the data operator (gene bank) and the end-user can verify its presence using any computational device, such a mobile phone.
